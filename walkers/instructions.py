...
class InstructionsFinder(object):
	...
	def _genBlockList(self, methods):
		for (cname, mname, minsts) in methods:
			...
			b = Block(etiqueta=cname + " " + mname + " 1", instrucciones=[], clase_padre = cname, metodo_padre = mname)
			for i2 in minsts:
				...
				if re.search("^goto", i2) != None:
					...
					(outCall, incetiqueta, posetiqueta) = self._splitBlock(b, cname, mname, blockPos, 1, minsts, i2)
					b.bifurcaciones = [('true',posetiqueta), ('false',incetiqueta)]
					b = self.add_before(etiqueta=incetiqueta, inst=i2, block=b, pclass=cname, pmethod=mname)
					...
					(outCall, incetiqueta, posetiqueta) = self._splitBlock(b, cname, mname, blockPos, 1, minsts, i2)
					b.bifurcaciones = [('exception',posetiqueta), ('try',incetiqueta)]
					b = self.add_before(etiqueta=incetiqueta, inst=i2, block=b, pclass=cname, pmethod=mname)
	...
	def do_find(self, where):
		...
		if class_name not in where: return []
