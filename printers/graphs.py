...
class GraphManager(object):
    ...
    def add_xref_edge(self, b1, b2, main_methods, coincidences=None):
        ...
        for strn in strs:
            if strn in main_methods:
                n = pydot.Node(strn, color="red", style="filled",
                               shape="record", fontname="Sans", fontsize=8)
            elif coincidences is not None:
                if strn in coincidences:
                    n = pydot.Node(strn, style="filled", shape="record",
                                   color="cornflowerblue", fontname="Sans", fontsize=8)
                else:
                    n = pydot.Node(strn, style="filled", shape="record",
                                   color="#eeeeee", fontname="Sans", fontsize=8)
            else:
                n = pydot.Node(strn, style="filled", shape="record",
                               color="#eeeeee", fontname="Sans", fontsize=8)
        ...
        self.graph.add_edge(pydot.Edge(
            nodes[0], nodes[1], color="#666666", arrowhead="open", fontsize=2))
    ...
    def add_block_edge(self, b1, b2, label="CONT"):
        ...
        blocks = [b1, b2]
        for i in range(2):
            if re.search("^if-", blocks[i].instrucciones[-1]) != None:
                ncolor = "cornflowerblue"
            elif re.search("^\:", blocks[i].instrucciones[0]) != None:
                ncolor = "tan"
            elif re.search("^goto", blocks[i].instrucciones[-1]) != None:
                ncolor = "darkgreen"
            elif re.search("^invoke-", blocks[i].instrucciones[-1]) != None:
                ncolor = "lightyellow4"
            else:
            ...
            for ii in blocks[i].instrucciones:
                insTxt += str(lidx) + ": " + ii
                lidx += 1

            nodes[i] = pydot.Node(b1.etiqueta, color=ncolor, style="filled",
                                  shape="box", fontname="Courier", fontsize=8)
            nodes[i].set_name(insTxt)
            self.graph.add_node(nodes[i])

        ed = pydot.Edge(nodes[0], nodes[1], color=ecolor, label=label,
                        fontname="Courier", fontsize=8, arrowhead="open")
